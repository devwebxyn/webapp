...
const PreviewModal: React.FC<{ file: DriveFile | null, onClose: () => void }> = ({ file, onClose }) => {
    ...
    const renderPreviewContent = () => {
        ...
        // Fallback untuk PDF menggunakan embed URL
        if (file.mimeType === 'application/pdf' && embedUrl) {
            return <iframe src={embedUrl} className="w-full h-full border-0 rounded-md" title={file.name} />;
        }
        
        // Tampilan Fallback untuk file yang tidak dapat dipratinjau
        ...
        // Fallback untuk Google Docs/Sheets/Slides dan file lainnya
        return (
            <div className="text-center text-neutral-300 flex flex-col items-center justify-center gap-4 p-8 h-full">
                <div className="text-7xl opacity-50">{getFileIcon(file.mimeType)}</div>
                <h3 className="text-xl font-bold">Pratinjau Langsung Tidak Tersedia</h3>
                    ...
                    >
                        <div className="py-1 text-sm text-neutral-200">
                            ...
                            >
                                ...
                                {file.isFeatured ? "Sembunyikan dari Share" : "Tampilkan di Share"}
                            </button>
                            {file.webViewLink && (
                                <a 
                                    href={file.webViewLink} 
                                    ...
                                    className="w-full text-left flex items-center gap-3 px-4 py-2 hover:bg-zinc-700"
                                >
                                    <VscOpenPreview /> Buka di Drive
                                </a>
                            )}
                            <div className="my-1 h-px bg-white/10" />
                            <button 
                                onClick={onDelete} 
                                className="w-full text-left flex items-center gap-3 px-4 py-2 text-red-400 hover:bg-red-500/20"
                            >
                                <VscTrash /> Hapus
                            </button>
                        </div>
    ...
    return (
        <div className="p-4 md:p-8">
            ...
            <UploadDestinationModal 
                ...
                onUpload={(file, folderId) => uploadFile(file, folderId, () => {})} 
            />
            <RenameModal 
                isOpen={showRenameModal} 
                onClose={() => setShowRenameModal(false)} 
                onRename={handleConfirmRename} 
                currentItemName={itemToModify?.name || ''} 
                isLoading={isLoading}
            />
            <PreviewModal 
                file={previewFile} 
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
                <input 
                    ...
                    className="flex-grow bg-white/5 border border-white/10 rounded-md p-3 pl-4 text-white" 
                />
                <div className="flex items-center gap-2">
                    ...
                    <button 
                        ...
                        className={`p-3 rounded-md ${
                            viewMode === 'list' ? 'bg-primary text-background' : 'bg-white/10 hover:bg-white/20'
                        }`}
                    >
                    ...
                    <button 
                        ...
                        className={`p-3 rounded-md ${
                            viewMode === 'grid' ? 'bg-primary text-background' : 'bg-white/10 hover:bg-white/20'
                        }`}
                    >
                ...
                <button 
                    ...
                    disabled={isLoading} 
                    className="flex items-center justify-center gap-2 bg-white/10 text-white font-bold p-3 rounded-md hover:bg-white/20"
                >
                ...
                <button 
                    ...
                    disabled={isLoading} 
                    className="flex items-center justify-center gap-2 bg-primary text-background font-bold p-3 rounded-md hover:bg-primary/80"
                >
            ...
            <div>
                {isLoading && files.length === 0 && (
                    <div className="text-center p-12">
                        <VscLoading className="animate-spin inline-block text-4xl text-primary" />
                    </div>
                )}
                {!isLoading && files.length === 0 && (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
                        {files.map(file => (
                            ...
                            >
                                ...
                                <p className="w-full font-semibold text-white text-sm break-words line-clamp-2">
                                    {file.name}
                                </p>
                        ...
                        <table className="w-full text-left table-fixed">
                            <thead className="border-b border-white/10">
                                <tr className="text-xs text-neutral-400 uppercase">
                                    <th className="p-4 w-1/2">Nama</th>
                                    <th className="p-4 w-1/4">Status</th>
                                    <th className="p-4 w-1/4 text-right">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                {files.map(file => (
                                    <tr key={file.id} className="border-b border-white/5 last:border-b-0 hover:bg-white/5 transition-colors">
                                        <td className="p-4 font-semibold text-white">
                                            ...
                                            >
                                                <div className="flex-shrink-0">
                                                    {file.mimeType.includes('folder') ? (
                                                        <VscFolder className="text-primary"/>
                                                    ) : (
                                                        <VscFile className="text-neutral-400"/>
                                                    )}
                                                </div>
                                        ...
                                        <td className="p-4">
                                            <span className={`flex items-center gap-2 text-xs px-2 py-1 rounded-full font-semibold w-fit ${
                                                file.shared ? 'bg-green-500/20 text-green-300' : 'bg-zinc-700 text-neutral-300'
                                            }`}>
                                                {file.shared ? <VscGlobe /> : <VscLock />} 
                                                {file.shared ? 'Publik' : 'Pribadi'}
                                            </span>
                                        </td>
            ...
            {hasMore && !isLoading && (
                <div className="text-center mt-8">
                    <button 
                        ...
                        className="text-primary hover:underline disabled:text-neutral-500" 
                        disabled={isLoading}
                    >
